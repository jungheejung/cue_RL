...
lb{1}=[0 0 0 0];
ub{1}=[1 1 100 100];
% mdl2) (5 parameters) 'mdl2 (confirmation bias with one weight)': (1) alpha_c
% (2) alpha_i (3) constant weight: w (4) painerror (5) experror
lb{2}=[0 0 0 0 0];
ub{2}=[1 1 1 100 100];
% mdl3)(4 parameters) 'mdl3 (one alpha with changing weight)':  (1) one alpha
% (2) one changing weight: w=2/(1+exp(gamma*|N-E|)). So parameters are
% alpha and gamma and (3) painerror (4) experror:
lb{3}=[0 0 0 0];
ub{3}=[1 100 100 100];
% mdl4)(5 parameters) 'mdl4 (confirmation bias with one changing weight)': (1)
...
% mdl5)(4 parameters) 'mdl5 (one alpha with changing weight)':  (1) one alpha
% (2) one changing weight: w=2/(1+exp(gamma*|N-E|)). So parameters are
% alpha and gamma and (3) painerror (4) experror. 
% w is the weight of N here
lb{5}=[0 0 0 0];
ub{5}=[1 100 100 100];
% mdl6)(5 parameters) 'mdl6 (confirmation bias with one changing weight)': (1)
% (1) alpha_c (2) alpha_i (3) one changing weight: w=2/(1+exp(gamma*|N-E|)). So parameters are
% alpha_c and alpha_i and gamma and % (4) painerror (5) experror
% w is the weight of N here
lb{6}=[0 0 0 0 0];
ub{6}=[1 1 100 100 100];
model_type={'mdl1', 'mdl2', 'mdl3', 'mdl4', 'mdl5', 'mdl6'};
...
High_Cue_subj=High_Cue{ii};
        ...
        m=1;
        for model_type={'mdl1', 'mdl2', 'mdl3', 'mdl4', 'mdl5', 'mdl6'}
...
for ii=1:size(aic,1)
    ...
    aic_mat(ii,4)=aic(ii,4,idx_best(1));
        idx_best=find(aic(ii,5,:)==min(aic(ii,5,:)));
    aic_mat(ii,5)=aic(ii,5,idx_best(1));
        idx_best=find(aic(ii,6,:)==min(aic(ii,6,:)));
    aic_mat(ii,6)=aic(ii,6,idx_best(1));
...
for ii=1:size(nll,1)
    ...
    nll_mat(ii,4)=nll(ii,4,idx_best(1));
        idx_best=find(nll(ii,5,:)==min(nll(ii,5,:)));
    nll_mat(ii,5)=nll(ii,5,idx_best(1));
        idx_best=find(nll(ii,6,:)==min(nll(ii,6,:)));
    nll_mat(ii,6)=nll(ii,6,idx_best(1));
end
...
column_names = {'mdl1: w constant & no biased learning', ...
                ...
                'mdl5: (W N) w change & no biased learning', ...
                'mdl6: (W N) w change & biased learning'};
bar(mean(nll_mat))
...
for jj=2:-1:1
    for ii=1:3
      ...
      couunt=couunt+1;
      sgtitle('demeaned expectation rating in the pain task: model verus behavior')
    end
end
...
for jj=2:-1:1
    for ii=1:3
      ...
      subtitle(noxious_title{ii})
      couunt=couunt+1;
      sgtitle('demeaned pain rating in the pain task: model verus behavior')
    end
end
...
close all
clc
load('pain_model_fitting_NLL.mat');
...
sgtitle('Pain task')
%% have a look at the congruent versus incongruent trials:
% calculate the number of congruent trials in which (cue=high and PE>0 | cue=low and
% PE<0) and incongruent trials in which (cue=high and PE<0 | cue=low and
% PE>0) and compare these two numbers

idx_congruent=(table_pain_new.param_cue_type=="high_cue" & table_pain_new.PE_mdl2>0) | (table_pain_new.param_cue_type=="low_cue" & table_pain_new.PE_mdl2<0);
sum(idx_congruent)
idx_incongruent=(table_pain_new.param_cue_type=="high_cue" & table_pain_new.PE_mdl2<0) | (table_pain_new.param_cue_type=="low_cue" & table_pain_new.PE_mdl2>0);
sum(idx_incongruent)

%% making a table of the parameters
load('param_mdl2_pain.mat');
load('subj_num_new_pain.mat');
alpha_c=param_mdl2_pain(1,:)';
alpha_i=param_mdl2_pain(2,:)';
weight_exp=param_mdl2_pain(3,:)';
pain_err=param_mdl2_pain(4,:)';
exp_err=param_mdl2_pain(5,:)';
par_mdl2_pain=table(subj_num_new_pain,alpha_c,alpha_i,weight_exp,pain_err,exp_err);
writetable(par_mdl2_pain,'par_mdl2_pain.csv');




%% what percentage of trials are congrunt for each participant? (based on model 2)
load("table_pain.mat");
subj_num_new=unique(table_pain.src_subject_id);
for ii=1:length(subj_num_new)
    sub_no=subj_num_new(ii);
Data=table_pain(table_pain.src_subject_id==sub_no,:);
param_cue_type = Data.param_cue_type;
Pain_mdl2 = Data.Pain_mdl2;
Exp_mdl2 = Data.Exp_mdl2;

% Logical indexing based on your conditions
high_cue_rows = strcmp(param_cue_type, 'high_cue') & Pain_mdl2 > Exp_mdl2;
low_cue_rows = strcmp(param_cue_type, 'low_cue') & Pain_mdl2 < Exp_mdl2;

% Combine the conditions using logical OR
selected_rows = high_cue_rows | low_cue_rows;
cong_trials_percentage(ii)=sum(selected_rows)/size(selected_rows,1);
end

histogram(cong_trials_percentage,20)
hold on
xline(mean(cong_trials_percentage),'r')
hold on
xline(median(cong_trials_percentage),'--r')
title('histogram of the percentage of congruent trials for each subejct')
