...
% 3)	Calculate the SSE based on the above equation
% 4)	For jj=1:20 % run 20 times to avoid being stuck in the 
% Optimize the SSE function and get the parameters and SSE for each model and pick the best parameter
% End
% 5)	Calculate aic, bic, and SSE or each model and compare different models together
%% do the model fitting part (SSE=SSE_Pain)
clear
close all
clc
...
% Calculate the SSE based on the above equation
...
High_Cue_subj=High_Cue{ii};
        ...
        for model_type={'mdl1', 'mdl2', 'mdl3', 'mdl4'}
            fit_fun=@(xpar)SSE_pain(xpar, Data, model_type{1},Noxious_subj,High_Cue_subj);
            for jj=1:run_numbers
                initpar=lb{m}+rand(1,length(lb{m})).*(ub{m}-lb{m});
                [qpar{ii,m,jj}, sse(ii,m,jj), bic(ii,m,jj), aic(ii,m,jj)]=fit_fun_sse(Data,fit_fun,initpar,lb{m},ub{m});
            end
            m=m+1;
        end
end

save pain_model_fitting_sse_pain
...
for ii=1:size(sse,1)
    idx_best=find(sse(ii,1,:)==min(sse(ii,1,:)));
    sse_mat(ii,1)=sse(ii,1,idx_best(1));
        idx_best=find(sse(ii,2,:)==min(sse(ii,2,:)));
    sse_mat(ii,2)=sse(ii,2,idx_best(1));
        idx_best=find(sse(ii,3,:)==min(sse(ii,3,:)));
    sse_mat(ii,3)=sse(ii,3,idx_best(1));
        idx_best=find(sse(ii,4,:)==min(sse(ii,4,:)));
    sse_mat(ii,4)=sse(ii,4,idx_best(1));
end

% compare model 1 and 3 in terms of aic and sse:
% aic:
figure
...
% sse:
figure
histogram(sse_mat(:,2)-sse_mat(:,4),20)
title('sse difference between model 2 and model 4')

% comparing model 1 and 2 in terms of aic and sse
% aic:
figure
...
% sse:
figure
histogram(sse_mat(:,1)-sse_mat(:,2),200)
title('sse difference between model 1 and model 2')
...
xtickangle(45);
ylim([2.5e4,2.7e4])
title('mean of sse in each model')
...
first_exp_low_cue(jj) = Data.event02_expect_angle(idx_ExpL);
        ...
        for ii=1:num_trial
            if High_Cue{jj}(ii)==1
                ...
                PainL{jj}(ii,1)=nan;
                SSEs{jj}(ii) = (Pain{jj}(ii,1)-Data.event04_actual_angle(ii))^2+(ExpectationH{jj}(ii,1)-Data.event02_expect_angle(ii))^2;
            else
                ...
                PainH{jj}(ii,1)=nan;
                SSEs{jj}(ii) = (Pain{jj}(ii,1)-Data.event04_actual_angle(ii))^2+(ExpectationL{jj}(ii,1)-Data.event02_expect_angle(ii))^2;
            end
...
%% looking at the SSEs in each trial:
clear
close all
clc
load('pain_model_fitting.mat');
...
Noxious_subj=Noxious{ii};
High_Cue_subj=High_Cue{ii};
[SSEs1,Pain1,Expectation1,PE1,SSEs_pain1,SSEs_exp1]=SSE_trialwise(param{1}(:,ii), Data, 'mdl1',Noxious_subj,High_Cue_subj);
[SSEs2,Pain2,Expectation2,PE2,SSEs_pain2,SSEs_exp2]=SSE_trialwise(param{2}(:,ii), Data, 'mdl2',Noxious_subj,High_Cue_subj);
[SSEs3,Pain3,Expectation3,PE3,SSEs_pain3,SSEs_exp3]=SSE_trialwise(param{3}(:,ii), Data, 'mdl3',Noxious_subj,High_Cue_subj);
[SSEs4,Pain4,Expectation4,PE4,SSEs_pain4,SSEs_exp4]=SSE_trialwise(param{4}(:,ii), Data, 'mdl4',Noxious_subj,High_Cue_subj);

            SSE_mdl1_trialwise=[SSE_mdl1_trialwise;SSEs1];
            SSE_mdl2_trialwise=[SSE_mdl2_trialwise;SSEs2];
            SSE_mdl3_trialwise=[SSE_mdl3_trialwise;SSEs3];
            SSE_mdl4_trialwise=[SSE_mdl4_trialwise;SSEs4];
            ...
            painsse1(ii)=SSEs_pain1;
            expsse1(ii)=SSEs_exp1;
            painsse2(ii)=SSEs_pain2;
            expsse2(ii)=SSEs_exp2;
            painsse3(ii)=SSEs_pain3;
            expsse3(ii)=SSEs_exp3;
            painsse4(ii)=SSEs_pain4;
            expsse4(ii)=SSEs_exp4;

      
end


table_pain_new.SSE_mdl1=SSE_mdl1_trialwise;
table_pain_new.SSE_mdl2=SSE_mdl2_trialwise;
table_pain_new.SSE_mdl3=SSE_mdl3_trialwise;
table_pain_new.SSE_mdl4=SSE_mdl4_trialwise;
...
%%%%%% looking at trialwise sse for each subject:
% for ii=1:length(subj_num_new)
for ii=1:20
sub_no=subj_num_new(ii);
Data=table_pain_new(table_pain_new.src_subject_id==sub_no,:);
...
figure
plot(time,Data.SSE_mdl2)
hold on
plot(time,Data.SSE_mdl4,'r')
xline(time(idx+1), 'k--')
legend('mdl2','mdl4')
end
...
idx = find(diff(Data.session_id) ~= 0);
figure
plot(time,Data.SSE_mdl4-Data.SSE_mdl2)
xline(time(idx+1), 'k--')
end

% violin plots of the SSEs in 6 conditions:
...
dat1 = table_pain_new.SSE_mdl2(idx_low_cue_low_stim);
% 2) low intensity high cue
dat2 = table_pain_new.SSE_mdl2(idx_high_cue_low_stim);
% 3) med intensity low cue
dat3 = table_pain_new.SSE_mdl2(idx_low_cue_med_stim);
% 4) med intensity high cue
dat4 = table_pain_new.SSE_mdl2(idx_high_cue_med_stim);
% 5) high intensity low cue
dat5 = table_pain_new.SSE_mdl2(idx_low_cue_high_stim);
% 6) high intrnsity high cue
dat6 = table_pain_new.SSE_mdl2(idx_high_cue_high_stim);
% beta mag
...
title('SSEs of model 2')


% SSE_pain and SSE_exp for subjects:
% mdl1
figure
Labels_pain_exp=[repelem([{'pain sse'}, {'exp sse'}], ...
    [length(painsse1), length(expsse1)])]';
data_pain_exp=[painsse1,expsse1]';
boxplt = boxplot(data_pain_exp, Labels_pain_exp);
set(boxplt, 'LineWidth', 1.5);
title('SSEs of model 1')
% mdl2
figure
Labels_pain_exp=[repelem([{'pain sse'}, {'exp sse'}], ...
    [length(painsse2), length(expsse2)])]';
data_pain_exp=[painsse2,expsse2]';
boxplt = boxplot(data_pain_exp, Labels_pain_exp);
set(boxplt, 'LineWidth', 1.5);
title('SSEs of model 2')
% mdl3
figure
Labels_pain_exp=[repelem([{'pain sse'}, {'exp sse'}], ...
    [length(painsse3), length(expsse3)])]';
data_pain_exp=[painsse3,expsse3]';
boxplt = boxplot(data_pain_exp, Labels_pain_exp);
set(boxplt, 'LineWidth', 1.5);
title('SSEs of model 3')
% mdl4
figure
Labels_pain_exp=[repelem([{'pain sse'}, {'exp sse'}], ...
    [length(painsse4), length(expsse4)])]';
data_pain_exp=[painsse4,expsse4]';
boxplt = boxplot(data_pain_exp, Labels_pain_exp);
set(boxplt, 'LineWidth', 1.5);
title('SSEs of model 4')
