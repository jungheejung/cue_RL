%% Model Fitting, Vicarious Pain perception:
...
table_pain=readtable([PATH_long '\spacetop_cue_vicarious_0405.csv']);
...
save vicarious_model_fitting_NLL
...
for jj=2:-1:1
    for ii=1:3
      ...
      couunt=couunt+1;
      sgtitle('demeaned expectation rating in the vicarious task: model verus behavior')
    end
end
...
for jj=2:-1:1
    for ii=1:3
      ...
      subtitle(noxious_title{ii})
      couunt=couunt+1;
      sgtitle('demeaned pain rating in the vicarious task: model verus behavior')
    end
end
...
close all
clc
load('vicarious_model_fitting_NLL.mat');
...
sgtitle('Vicarious task')

%% making a table of the parameters
load('param_mdl2_vicarious.mat');
load('subj_num_new_vicarious.mat');
alpha_c=param_mdl2_vicarious(1,:)';
alpha_i=param_mdl2_vicarious(2,:)';
weight_exp=param_mdl2_vicarious(3,:)';
pain_err=param_mdl2_vicarious(4,:)';
exp_err=param_mdl2_vicarious(5,:)';
par_mdl2_vic=table(subj_num_new_vicarious,alpha_c,alpha_i,weight_exp,pain_err,exp_err);
writetable(par_mdl2_vic,'par_mdl2_vic.csv');